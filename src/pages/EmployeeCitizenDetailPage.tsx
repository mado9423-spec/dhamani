...
export default function EmployeeCitizenDetailPage() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#F5F6F7] pb-10 font-cairo">
      <header className="flex items-center gap-3 bg-[#1A1D21] px-6 py-4">
      ...
      <main className="flex flex-col gap-4 px-6 py-5">
        <section className="rounded-2xl border border-[#E5E7EB] bg-white p-5">
          ...
          <p className="mt-1 text-base font-bold text-[#1A1D21]">{citizen.full_name}</p>
          <div className="my-3 h-px bg-[#E5E7EB]" />
          <p className="text-xs font-semibold text-[#6B7280]">رقم المعاش</p>
          <p className="mt-1 text-base font-bold text-[#1A1D21]">{citizen.pension_number}</p>
          <div className="my-3 h-px bg-[#E5E7EB]" />
          <p className="text-xs font-semibold text-[#6B7280]">الحالة</p>
          <p className="mt-1 text-base font-bold text-[#16794F]">
            {citizen.status === "active" ? "نشط" : citizen.status === "suspended" ? "موقوف" : "مؤرشف"}
          </p>
        </section>

        <section>
          <h2 className="mb-2 text-sm font-bold text-[#1A1D21]">الإقرارات السنوية</h2>
          {declarations.length === 0 ? (
            <p className="text-[13px] font-medium text-[#9CA3AF]">لا توجد إقرارات مسجّلة</p>
          ) : (
            <div className="flex flex-col gap-2">
              {declarations.map((d) => (
                <div key={d.id} className="rounded-xl border border-[#E5E7EB] bg-white p-3 text-[13px] font-semibold text-[#1A1D21]">
        ...
        <section>
          <h2 className="mb-2 text-sm font-bold text-[#1A1D21]">المواعيد</h2>
          {appointments.length === 0 ? (
            <p className="text-[13px] font-medium text-[#9CA3AF]">لا توجد مواعيد</p>
          ) : (
            <div className="flex flex-col gap-2">
              {appointments.map((a) => (
                <div key={a.id} className="rounded-xl border border-[#E5E7EB] bg-white p-3 text-[13px] font-semibold text-[#1A1D21]">
        ...
        <section>
          <h2 className="mb-2 text-sm font-bold text-[#1A1D21]">المعاملات</h2>
          {transactions.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {transactions.map((t) => (
                ...
                >
                  <span className="text-[13px] font-semibold text-[#1A1D21]">
        ...
        <section className="rounded-2xl border border-[#E5E7EB] bg-white p-5">
          <h2 className="mb-3 text-sm font-bold text-[#1A1D21]">إنشاء معاملة جديدة</h2>
          ...
          <label htmlFor="notes" className="mb-1.5 mt-4 block text-sm font-semibold text-[#1A1D21]">
            ملاحظات (اختياري)
          </label>
          <textarea
            id="notes"
            ...
            rows={3}
            className="w-full rounded-xl border border-[#E5E7EB] p-3 text-sm font-medium text-[#1A1D21] outline-none focus:border-[#0B3D66]"
          />
